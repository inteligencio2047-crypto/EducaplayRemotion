...
export const SlotContainer: React.FC<SlotContainerProps> = ({
  ...
  // Smooth entrance spring according to motion-designer principles
  const spr = spring({
    frame,
    fps,
  ...
  // 1. small_left: Teacher centered -> Resource on LEFT (compact ~540px)
  // 2. medium_right: Teacher on LEFT -> Resource on RIGHT (medium ~680px)
  // 3. large_center: Teacher OFF / Fullscreen -> Resource in CENTER (large ~1180px)
  ...
  if (!containerMaxWidth) {
    if (effectiveAnchor === 'left') containerMaxWidth = 600;
    else if (effectiveAnchor === 'right') containerMaxWidth = 720;
    else containerMaxWidth = 1180;
  }

  const containerMaxHeight = maxHeight || 640;

  // Top position calculation:
  // For 'right' anchor: push down to 280px to safely clear the top-right mosca logo
  // For 'left' anchor: align at 240px
  // For 'center': center vertically
  let topPos = '240px';
  if (effectiveAnchor === 'center') {
    topPos = `${(height - containerMaxHeight) / 2 - 30}px`;
